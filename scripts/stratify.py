...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Stratify phenotype and methylation data"
    )
    parser.add_argument(
        "--pheno",
        required=True,
    ...
    parser.add_argument(
        "--methyl",
        required=True,
        help="Path to methylation matrix",
    )
    parser.add_argument(
        "--stratify",
        nargs="+",
        required=True,
        help="List of variables to stratify by",
    )
    ...
    for group_vals, sub_pheno in groups:
        group_name = (
            "_".join(str(v) for v in group_vals)
